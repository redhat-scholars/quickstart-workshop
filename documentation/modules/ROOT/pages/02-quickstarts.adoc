...
6. Download a copy of the `install-helmchartrepo-ns` quickstart:
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
7. Update the quickstart's included `url` attribute with the Bitnami helm repo address: `https://charts.bitnami.com/bitnami`
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
8. Update the quickstart's name / id attribute with a unique value:
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
9. Upload the updated quickstart:
+
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
...
[#helm_charts]
== 2.3 Check your work - Helm Charts per project
...
1. Click on the "?" icon in the header menu, Then select "Quick Starts".  Next, Search for "Helm"
2. Select the tile labeled "Add Helm Chart Repositories to extend the Developer Catalog for your project"
3. Complete each step of the quickstart.  Upon completion, confirm that the Bitnami helm repo is successfully installed within the project namespace
